...
contract ERC20 is AccessControl {
    ...
    function transfer(address to, uint256 value) external enoughTokens(msg.sender, value) returns (bool) {
    ...
    function transferFrom(address from, address to, uint256 value) external enoughTokens(from, value) returns (bool) {
        require(to != address(0), "Can't transfer to zero address");
    ...
    function burn(address account, uint256 amount) external onlyRole(BURNER_ROLE) enoughTokens(account, amount){
